...
preprocess.py
-------------
Load corrected CIC-IDS2017 CSVs, clean, encode, split, and build the
per-feature CONSTRAINT SPEC that the constraint-aware attack (Day 3) needs.
"""
...
def clean(df):
    ...
    # Drop non-semantic / label-correlated columns: a row identifier ("id") and
    # a bookkeeping column ("Attempted Category") that do not correspond to flow
    # measurements. Set EXTRA_HYGIENE=True to also drop Src/Dst Port as features.
    EXTRA_HYGIENE = False
    leaky_exact = ["id", "attempted category"]
    if EXTRA_HYGIENE:
        leaky_exact += ["src port", "dst port", "source port", "destination port"]
    leaky = [c for c in X.columns if _norm(c) in leaky_exact]
    to_drop = list(dict.fromkeys(to_drop + leaky))
    if to_drop:
        X = X.drop(columns=to_drop)
        print(f"[clean] dropped id-like/non-semantic columns: {to_drop}")
