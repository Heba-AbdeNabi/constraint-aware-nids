"""
run_arch_and_baseline.py
========================
Produces TWO reviewer-requested comparisons in one pass, for both binary and
multiclass:

  EXPERIMENT 1 - Architecture robustness (MLP vs 1D-CNN victim):
    For each architecture, train an undefended victim and a constrained-
    adversarially-trained defended model, and measure clean / unconstrained /
    constrained accuracy. Shows the paper's findings replicate across
    architectures.
    -> arch_compare.csv

  EXPERIMENT 2 - Training-type baseline (per architecture):
    Compare undefended vs UNCONSTRAINED adv-training vs CONSTRAINED adv-training
    on the realizable (constrained) attack. Shows constrained training is the
    right choice, not just "adversarial training helps".
    -> baseline_compare.csv

Reuses your existing preprocess.py / attack.py. Self-contained training loops so
it does not depend on cached models. Just Run in PyCharm.

Time: heaviest script so far (2 architectures x several trainings x 2 modes).
Budget ~1-2 h on the RTX 8000. Trim MODES or ARCHES below for a faster pass.
"""
